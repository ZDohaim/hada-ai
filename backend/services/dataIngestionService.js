...
const SEARCH_TERMS = {
  floward: {
    gifts: ["gifts", "bouquet", "flowers", "roses", "arrangements"],
    premium: ["luxury", "premium", "exclusive", "elegant", "collection"],
    fashion: ["jewelry", "accessories", "watches", "fashion"],
    perfume: ["perfume", "fragrance", "scents", "cologne"],
    home_decor: ["home", "decor", "vases", "candles"]
  },
  jarir: {
...
const ingestFromFloward = async (searchTerm, category, limit = 50) => {
  console.log(`→ Fetching FLOWARD products for "${searchTerm}" (${category})`);
  ...
  try {
    ...
    if (!response.data || !Array.isArray(response.data)) {
      console.log(`⚠️ FLOWARD: No products found for "${searchTerm}"`);
      return [];
    }

    // Limit results and normalize products
    const products = response.data.slice(0, limit);
    const normalizedProducts = products
      .map(p => normalizeFlowardProduct(p, category))
      .filter(p => p && p.price_sar > 0);

    console.log(`✓ FLOWARD: ${normalizedProducts.length} valid products for "${searchTerm}"`);
    return normalizedProducts;

  } catch (error) {
    console.error(`❌ FLOWARD API error for "${searchTerm}":`, error.message);
    return [];
  }
};
...
const normalizeFlowardProduct = (product, category) => {
  ...
  // Extract image URL
  const imageUrl = product.images?.coverImage || 
  ...
  // Extract brand information
  const brand = product.attributes?.brand?.[0]?.nameEn || 
                product.brand || 
                null;
